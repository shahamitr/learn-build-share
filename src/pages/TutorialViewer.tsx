        ...
        <AnimatePresence initial={false}>
          {showSidebar && (
            <motion.div 
              ...
              className="bg-white border-r border-slate-200 overflow-y-auto hidden md:block"
            >
              <div className="p-6">
                ...
                {curriculum.map((level) => (
                  <div key={level.id} className="mb-6">
                    <h4 className="text-sm font-semibold text-slate-900 mb-2">{level.title}</h4>
                    <ul className="space-y-1">
                      {level.modules.map((m) => (
                        <li key={m.id}>
                          <Link 
                            to={`/tutorials/${courseId}/${m.id}`}
